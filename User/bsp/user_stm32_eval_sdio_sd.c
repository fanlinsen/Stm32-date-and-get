...
/* Includes ------------------------------------------------------------------*/
#include "user_stm32_eval_sdio_sd.h"
...
/* Private variables ---------------------------------------------------------*/
static uint32_t CardType =  SDIO_STD_CAPACITY_SD_CARD_V1_1; //存储卡的类型，先将其初始化为版本1.1协议的卡
...
{ 
  ...
  SDIO_ClockCmd(DISABLE);                                    //关闭SDIO时钟
  SDIO_SetPowerState(SDIO_PowerState_OFF);                   //关闭电源控制状态
  SDIO_DeInit();                                             //初始化SDIO外设寄存器
  
  /*!< Disable the SDIO AHB Clock */
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_SDIO, DISABLE);  
  
  //SDIO数据线SDIO_D[3:0]引脚PC8、PC9、PC10、PC11配置
  GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_11;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;       //浮空输入
  GPIO_Init(GPIOC, &GPIO_InitStructure);

  //SDIO时钟线SDIO_CK引脚PC12配置
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;            //复用推挽输出
  GPIO_Init(GPIOC, &GPIO_InitStructure);
  
  //SDIO命令响应信号线SDIO_CMD引脚PD2配置
...
SD_Error SD_Init(void)
{
  ...
  SDIO_SD_Init();                      //初始化SDIO用到的引脚，开启时钟

  SDIO_DeInit();                       //初始化SDIO外设寄存器

  errorstatus = SD_PowerON();          //上电并进行卡识别流程，确认卡的操作电压

  if (errorstatus != SD_OK)            //如果上电，识别卡
  {
    return(errorstatus);               //如果识别不成功，返回“响应超时”错误
  }

  errorstatus = SD_InitializeCards();  //卡识别成功，进行卡初始化

  if (errorstatus != SD_OK)	           //再次识别卡
  {
    return(errorstatus);               //如果识别不成功，返回“响应超时”错误
  }
  ...
  SDIO_InitStructure.SDIO_ClockEdge = SDIO_ClockEdge_Rising;
	...
	/* 时钟频率若超过24M,要开启此模式 */
  SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;  //SDIOCLK分频驱动SDIO_CK输出
	
	/* 若开启此功能，在总线空闲时关闭sd_clk时钟 */
  SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;  //始终输出SDIO_CK
	
  /* 暂时配置成1bit模式 */	
  SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;   //1位总线模式，使用SDIO_D[0]

	/* 硬件流，若开启，在FIFO不能进行发送和接收数据时，数据传输暂停 */
  SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;//关闭硬件流控制
  SDIO_Init(&SDIO_InitStructure);
  
  if (errorstatus == SD_OK)
  {
    errorstatus = SD_GetCardInfo(&SDCardInfo);   //读取CSD/CID MSD寄存器
  }

  if (errorstatus == SD_OK)
  {
    /* 通过cmd7  ,rca选择要操作的卡 */
    errorstatus = SD_SelectDeselect((uint32_t) (SDCardInfo.RCA << 16));  //选卡操作
  }

  if (errorstatus == SD_OK)
  {
			/* 最后为了提高读写，开启4bits模式 */
    errorstatus = SD_EnableWideBusOperation(SDIO_BusWide_4b);//开启4位总线模式，使用SDIO_D[3：0]
  }  
...
SD_Error SD_PowerON(void)
{
  ...
  /********************************************************************************************************/
  //通过对SDIO_InitStructure结构体初始化实现对SDIO外设的配置  
  /* 上电初始化 
   * 配置SDIO的外设
  ...
  /* 开启SDIO外设的电源 */
  SDIO_SetPowerState(SDIO_PowerState_ON);                      //开启电源控制状态
  SDIO_ClockCmd(ENABLE);                                       //开启SDIO时钟
/********************************************************************************************************/   
  //通过对SDIO_CmdInitStructure结构体初始化发送配置命令 
  /* 下面发送一系列命令,开始卡识别流程
   * CMD0: GO_IDLE_STATE(复位所以SD卡进入空闲状态) 
   * 没有相应 
	 */
  SDIO_CmdInitStructure.SDIO_Argument = SD_STD_CAPACITY;        //命令参数，是命令的一部分，和命令一起发送
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_GO_IDLE_STATE;   //SD卡进入空闲状态
	/* 没有响应 */
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_No;       //关闭响应
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;               //关闭等待中断 
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;           //CPSM在开始发送命令之前等待数据传输结束
...
/********************************************************************************************************/
  ...
  SDIO_CmdInitStructure.SDIO_CmdIndex = SDIO_SEND_IF_COND;     //发送CMD8，核实SD卡接口操作条件
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;   //响应类型：R7
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;              //关闭等待中断 
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;          //CPSM在开始发送命令之前等待数据传输结束
  SDIO_SendCommand(&SDIO_CmdInitStructure);
  ...
  errorstatus = CmdResp7Error();                         //检查是否接收到命令
	...
	/* 有响应则card遵循sd协议2.0版本 */
  if (errorstatus == SD_OK)                              //判断识别卡是否有响应
  {
		/* SD Card 2.0 ，先把它定义会sdsc类型的卡 */
    CardType = SDIO_STD_CAPACITY_SD_CARD_V2_0;           //先把它定义会sdsc类型的卡，标准容量SD卡2.0 
    SDType = SD_HIGH_CAPACITY;                           //这个变量用作ACMD41的参数，用来询问是sdsc卡还是sdhc卡
  }
  else                                                   //无响应，说明是1.x的卡或者mmc的卡
  {
    /* 发命令 CMD55 */	  
    SDIO_CmdInitStructure.SDIO_Argument = SD_STD_CAPACITY;       //命令参数，是命令的一部分，和命令一起发送
    SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_APP_CMD;        //发送CMD55，判断是不是SD卡
    SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;   //响应类型：R1
    SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;              //关闭等待中断 
    SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;          //CPSM在开始发送命令之前等待数据传输结束
    SDIO_SendCommand(&SDIO_CmdInitStructure);
...
/********************************************************************************************************/
   ...
   * or SD card 1.x 
	 */
  if (errorstatus == SD_OK)                                     //响应了cmd55，是sd卡，可能是1.x，也可能是2.0
  {
    ...
    while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))
    {
       ...
       * 响应：R3,对应的是OCR寄存器			
			 */			
      SDIO_CmdInitStructure.SDIO_Argument = SD_VOLTAGE_WINDOW_SD | SDType;   //参数为主机可供电范围及hcs位
      SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SD_APP_OP_COND;           //发送CMD41，判断是SDSC卡还是SDHC卡
      SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;             //响应类型：R3
      SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;                        //关闭等待中断 
      SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;                    //CPSM在开始发送命令之前等待数据传输结束
      SDIO_SendCommand(&SDIO_CmdInitStructure);
      ...
      validvoltage = (((response >> 31) == 1) ? 1 : 0);                      //读取卡的ocr寄存器的pwr_up位，看是否工作在正常电压
      count++;			  /* 计算循环次数 */
    }
    //如果尝试了SD_MAX_VOLT_TRIAL多次，初始化还没有完成，则VDD电压窗口（3.2V~3.3V）是无效的
    if (count >= SD_MAX_VOLT_TRIAL)					 /* 循环检测超过一定次数还没上电 */
    {
      errorstatus = SD_INVALID_VOLTRANGE;                                    //SDIO不支持card的供电电压
      return(errorstatus);
    }
    //判断CCS位是否为1（CCS位为1，表示是高容量SD卡）
		/*检查卡返回信息中的HCS位*/
		/* 判断ocr中的ccs位 ，如果是sdsc卡则不执行下面的语句 */
    if (response &= SD_HIGH_CAPACITY)
    {
      CardType = SDIO_HIGH_CAPACITY_SD_CARD;                                 //把卡的类型从初始化的sdsc改为sdhc
    }
...
SD_Error SD_InitializeCards(void)
{
  ...
  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)                     //判断卡的类型
  {
    /* Send CMD2 ALL_SEND_CID 
		 * 响应：R2，对应CID寄存器
		 */
    SDIO_CmdInitStructure.SDIO_Argument = SD_STD_CAPACITY;
